...
doEdit(request, response);	
	...
	protected void doEdit(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			ConnectSQL c = new ConnectSQL();
			ResultSet re = c.getData("select * from nhanvien where id ="+id);
			re.next();
			request.setAttribute("employee", re);
			request.getRequestDispatcher("/admin/suaEmployee.jsp").forward(request, response);
		} catch (Exception e) {
